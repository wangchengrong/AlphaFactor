...
def plot_returns_table(alpha_beta,
    ...
    returns_table.loc['Mean Period Wise Return Top Quantiles (bps)'] = \
        mean_ret_quantile.iloc[-1] * DECIMAL_TO_BPS
    returns_table.loc['Mean Period Wise Return Bottom Quantiles (bps)'] = \
...
def plot_ic_ts(ic, ax=None):
    """
    Plots Spearman Rank Information Coeficient and IC moving
    ...
    if ax is None:
        f, ax = plt.subplot(num_plots, 1, figsize=(18, num_plots * 7))
...
def plot_ic_hist(ic, ax=None):
    ...
    if ax is None:
        f, ax = plt.subplots(v_spaces, 1, figsize=(18, v_spaces * 6))
        ax.flatten()
...
def plot_quantile_returns_bar(mean_ret_by_q,
    ...
    ylim_percentiles: tuple of integers, optional
    ...
    if by_group:
        ...
        for a, (sc, cor) in zip(ax, mean_ret_by_q.groupby(level='group')):
            ...
            a.set(xlabel='', ylabel='Mean Returns (bps)',
                  ylim=(ymin, ymax))
...
def plot_mean_quantile_returns_spread_time_series(mean_returns_spread,
    ...
    bandwidth: float
        Width of display error bands in standard deviations.
    ax: matplotlib.Axes, optional
        Axes upon which to plot.
    ...
    if isinstance(mean_returns_spread, pd.DataFrame):
        ...
        for (i, a), (name, fr_column) in zip(enumerate(ax),
            ...
            a = plot_mean_quantile_returns_spread_time_series(fr_column,
                                                              ...
                                                              ax=a)
            ax[i] = a
            curr_min, curr_max = a.get_ylim()
            ymin = curr_min if ymin is None else min(ymin, curr_min)
            ymax = curr_max if ymax is None else max(ymax, curr_max)
    ...
    mean_returns_spread_bps.rolling(window=22).mean().plot(color='orangered', alpha=0.7, ax=ax)
...
def plot_ic_by_group(ic_group, ax=None):
    """
    Plots Spearman Rank Information for a given factor over
...
def plot_factor_rank_auto_correlation(factor_autocorrelation,
    ...
    factor_autocorrelation.plot(title='{} Period Factor Rank Autocorrelation'.format(period),
                                ax=ax)
...
def plot_monthly_ic_heatmap(mean_monthly_ic, ax=None):
    ...
    mean_monthly_ic: pd.DataFrame
        The mean monthly IC for N period forward
    ax: matplotlib.Axes, optional
        Axes upon which to plot.
    ...
    for a, (period_num, ic) in zip(ax, mean_monthly_ic.iteritems()):
        ...
        a.set(ylabel='', xlabel='')
        a.set_title('Monthly Mean {} Period IC'.format(period_num))
...
def plot_cumulative_returns(factor_returns, period=1, overlap=True, ax=None):
    ...
    if ax is None:
        f, ax = plt.subplot(1, 1, figsize=(18, 6))
...
def plot_cumulative_returns_by_quantile(quantile_returns,
    ...
    ret_wide = quantile_returns.reset_index().pivot(index='date', columns='factor_quantile', values=period)

    overlapping_period = period if overlap else 1
    cum_ret = ret_wide.apply(utils.cumulative_returns, args=(overlapping_period,))
...
def plot_quantile_average_cumulative_return(avg_cumulative_returns,
    ...
    if by_quantile:
        ...
        for i, (quantile, q_ret) in enumerate(avg_cumulative_returns.groupby(level='factor_quantile')):
            ...
            ax[i].set_ylabel('Mean Returns (bps)')
        ...
        ax.legend()
        ax.set(ylabel='Mean Returns (bps)',
               title=('Average Cumulative Returns by Quantile'
                      if title is None else title), xlabel='Periods')
